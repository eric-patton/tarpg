...
| `Cleave.CooldownSec` / `Cost` / `Damage` | `1.0f` / `10` Rage / `14` (slight single-target edge over auto-attack; dominates clusters at 2+ adj) | `Skills/Cleave.cs` |
...
| `WarCry.CooldownSec` / `Cost` / `HealAmount` | `12.0f` / `25` Rage / `35` HP (round-1 balance pass; closes Reaver's F8+ sustain gap) | `Skills/WarCry.cs` |
...
- **Discovery: pilots were too dumb to be a balance baseline.** The greedy pilot never fired War Cry (E) and didn't drink potions — Reaver was "dying at depth" because the sim was approximating a player who'd forgotten their defensive cooldowns existed. Tuning content against that data would have over-buffed Reaver for actual players.
- **`Core/GameLoopController.cs`** — moved potion-drink logic out of `GameScreen` so sim pilots and live play hit the same code path. New `TryDrinkHealthPotion()` / `TryDrinkResourcePotion()` (return bool) own the cooldown gate, inventory consume, HP/Resource clamp. `GameScreen.TryDrinkXPotion` is now a thin wrapper that delegates and adds the screen-flash VFX. Drink cooldowns decay inside `loop.Tick`.
- **`Sim/GreedySimPilot.cs`** — casts War Cry every tick HP ≤ 50% (cooldown / Rage gates inside `TryCastSkill`); panic-drinks an HP potion below 30% as the last-ditch heal. F8-F10 deaths halved (14 → 7) and clear% jumped (F8 84%→92%, F9 76%→88%, F10 76%→80%) without touching a single content number.
- **`Sim/KitingSimPilot.cs`** — same panic-drink fallback below 30% HP. Currently never triggers in the data (Hunter's HpMin stays around 36 at F10), so the change is correctly idle on the kiter — but it's there for the deeper-floor scenarios this pilot will face when F11+ enters scope.
- **`Skills/Cleave.cs`** — bumped Damage 10 → 14. With auto-attack at 10 dmg / 0.8s = 12.5 DPS and Cleave at 14 dmg / 1.0s = 14 DPS, Cleave now slightly wins single-target instead of losing — addresses the "feels off at low enemy counts" open issue. Sim impact was negligible (greedy only fires Cleave on 2+ adjacent), but live play will see the single-target case differently.
- **`Skills/WarCry.cs`** — bumped HealAmount 25 → 35. The most impactful single content tweak in the round. Reaver was dying with ~165 dmg taken vs ~130 effective sustain pool (65 MaxHP + ~25 War Cry + ~40 potion); the +10 HP per cast plus typically 2 casts per fight closed most of the gap.
- **Final clear curves (n=25 seeds per floor)**:
  - **Reaver/greedy**: F1-F5 100% / F6-F7 96% / F8 92% / F9 88% / F10 80%, avg clear 87s
  - **Hunter/kiting**: F1-F5 100% / F6-F10 96% (1 timeout always on the seed-1023 BSP outlier), avg clear 95s
  - **Class signature**: Reaver = "win fast or die" (faster clears, meaningful death risk at F8+); Hunter = "patient survival" (slower clears, near-perfect survival)
- **Tests**: `Sim/GreedySimPilotTests.cs` adds low-HP-fires-War-Cry + critical-HP-drinks-potion. `Sim/KitingSimPilotTests.cs` adds critical-HP-drinks-potion (Bandage disabled to force the panic path). `Items/InventoryTests.cs` adds a `PotionDrinkTests` class covering `GameLoopController.TryDrinkHealthPotion` happy path, no-stock case, on-cooldown case, and cooldown-decay-allows-next-drink.
...
- **Why**: investigating "why does this seed time out" used to require either patching `TickRunner` to dump more state or writing a one-off C# script. `--dump-floor` makes the same investigation a one-liner: `sim --dump-floor --seed-base 1023 --floors 6`. No sim runs — just generation + reachability + viz.
- **First use**: confirmed seed 1023 F6+ is *not* a BSP connectivity bug — every enemy is A*-reachable from entry. The pilot timeouts are pilot navigation limits (Euclidean-nearest sticky targeting + `SkirmisherAi`'s FOV-gated aggro on tucked-away alcoves). Documented in Open Questions for the eventual balance-pass session.

### Hunter Focus regen + KitingSimPilot commit-fight
- **Discovered via first Hunter sweep** (`runs/run02-hunter-kiting.csv`): kiter cleared only 48–72% of floors vs Reaver/greedy's 48–100%. Most failures were timeouts (not deaths) — pilot just couldn't finish runs. Six pathological seeds (1004, 1009, 1011, 1016, 1018, 1022) timed out across every floor with `0 enemies killed, 0 dmg dealt, 0 dmg taken, 0 skill uses`.
- **Root cause #1 (game)**: Hunter had no Focus generation. Reaver's Rage regens via `GrantResourceOnHit` — but the kiter never auto-attacks, and there was no passive regen. So Roll, Volley, Bandage, RainOfArrows were *all* locked out forever and only QuickShot (cost=0) was usable. Hunter's tagline literally says "regenerating Focus" — the implementation was missing.
- **Root cause #2 (pilot)**: Even with QuickShot, when an enemy sat at cheby=2 with LOS blocked by a wall (e.g. an alcove neighboring a corridor), the kiter oscillated between "retreat" (cheby=2 → flee SW) and "close in" (cheby=3 → walk back) every tick, 1 tile of pacing in either direction, never gaining LOS to fire. Wolfshade couldn't quite reach melee adjacency due to its skirmisher AI cadence, so `dmg_taken` stayed 0 too. Player pinned forever, run timed out.
- **`Classes/WalkerClassDefinition.cs`** — added `PassiveResourceRegenPerSec` (default 0). **`Classes/Hunter.cs`** — Hunter sets it to 3.0 Focus/sec.
- **`Core/GameLoopController.cs`** — new `TickResourceRegen` runs each frame alongside `TickHpRegen`. Independent of the out-of-combat HP delay so Focus is *always* trickling, not just when safe. Reset on floor load; no overflow past `MaxResource`.
- **`Sim/KitingSimPilot.cs`** — added a commit-fight fallback: when the threat sits at cheby ≤ 3 (`IdealKiteDistanceMin`) without LOS for 180 ticks (3 sim-sec), the pilot sets `Combat.Target` so the loop's chase + auto-attack drives the engagement. Resets when LOS reopens (kiter resumes ranged behavior) or target dies. Threshold sits well below the existing 60-tick stuck-bail backstop, so the order is: pin → commit-fight → if STILL stuck, walk to threshold.
- **Also fixed**: close-in branch's M2 cast was firing whenever `hasLos` was true regardless of range; now gated on `threatCheby ≤ QuickShotMaxRange (6)` so the 0.5s cooldown isn't wasted on out-of-range casts.
- **Verified**: re-running the previously-pinned 1018 F2 now clears in 76.6s with 12/12 kills (was: 5-min timeout, 0 kills). Full 250-run Hunter sweep (`runs/run03-hunter-fixed.csv`) clears **246/250 (98.4%) with 0 deaths** — only 1 timeout per floor F6-F10, all on seed 1023 which **also times out for Reaver/greedy** on the same floors (a structural BSP-layout issue affecting both pilots, not kiter-specific). Side-by-side: at F10, Hunter/kiting 24/25 cleared vs Reaver/greedy 11/25 — kite-and-regen survives deep floors that the melee approach dies on (real signal: at F8+ the Reaver dies in 7-14/25 runs while Hunter never dies, just times out on the bad seed). Hunter is ~30% slower per floor (104s vs 80s at F10) — kite-band fire trades safety for clear-time.
- **Tests** — `Sim/KitingSimPilotTests.cs` adds pinned-by-wall-commits-to-fight + commit-resets-on-LOS-clear (drops the wall mid-run, asserts combat target clears). New `Core/ResourceRegenTests.cs` covers Hunter accumulates Focus over 10 sim-sec, Reaver does NOT passively regen Rage, and Hunter caps at MaxResource.

### KitingSimPilot — Hunter-flavored sim pilot
- **`Sim/KitingSimPilot.cs`** (new) — second `ISimPilot` implementation alongside `GreedySimPilot`. Maintains a 3–6 chebyshev kite band from the nearest enemy: stands still and fires QuickShot from range with LOS, retreats (Roll preferred, foot-flee fallback) when an enemy crosses into the danger zone (cheby ≤ 2), closes when the target is out of range or LOS-blocked. Drops Volley on 2+ clusters within radius-1, Rain of Arrows on 3+ clusters within radius-2, Bandage when HP ≤ 50%. Same sticky-target hysteresis (25%) and stuck-bail (60-tick threshold → walk to threshold) as Greedy.
- **Why no auto-attack**: the kiter never calls `Combat.SetTarget`. With a combat target set, `GameLoopController.Tick` chases the target via `Movement.RetargetTo` every tick — which directly fights kiting. All damage comes from skills. QuickShot (12 dmg / 0.5s cd ≈ 24 DPS) already beats Hunter's auto-attack baseline (10 dmg / 0.8s ≈ 12.5 DPS), so the kit doesn't lose much by skipping the swing.
- **Stuck-detector reset on intentional stillness**: kite-band-with-LOS is the kiter's "happy place" — standing still by design for many ticks. The bail counter explicitly resets in that branch so a successful kite never falsely degrades into "drop combat, walk to threshold" after a sim-second.
- **`Tarpg.Sim/Program.cs`** — `MakePilot` switch now routes `--pilot kiting` to `KitingSimPilot`; usage string + interactive prompt updated. Backward-compat: `--pilot greedy` stays the default.
- **Tests** — `Sim/KitingSimPilotTests.cs` covers no-enemies-routes-to-threshold, hold-and-shoot in kite band, never-sets-combat-target across 10 ticks, Roll-when-in-danger, foot-flee-when-Roll-on-cooldown, close-when-out-of-range, Bandage-at-low-HP, intentional-stillness-doesn't-bail (regression for the false-bail case), genuine-stuck-bails, AOE-on-cluster.

### Map.FindPath PathFinder cache (~12× per-tick speedup)
- **Diagnosis**: a 250-run sweep that should have taken ~1 minute under 24-way parallelism was projected for 30+ minutes. Profiling a single F1 seed=1000 run showed `GameLoopController.Tick` was 84% of wall-time at ~3.9 ms/tick; drilling further, **94% of that was the player's `RetargetTo + MovementController.Tick`** when the path-fallback fired. Each `Map.FindPath` call was constructing a fresh `RogueSharp.PathFinder` — and that constructor builds an `EdgeWeightedDigraph` over every walkable cell of the map.
- **`World/Map.cs`** — cache one `PathFinder` per Map; allocate lazily on the first `FindPath`, invalidate by setting the cache to `null` inside `SetTile` (the only code path that flips a walkability bit, and in practice only called during BSP generation). Subsequent `FindPath` calls reuse the cached graph and only pay Dijkstra cost.
- **Result**: per-tick dropped from **3.88 ms → 0.31 ms (12.6× faster)** on the F1 seed=1000 baseline. End-to-end, a 15-run F1–F3 × 5-seed parallel sweep went from **9m40s → 7.6s (~76× wall-clock)**, and the per-run `SimResult`s are byte-identical to pre-cache (PlayerHpAtEnd, kills, sim-seconds all match), so determinism is preserved.
- **Why it's safe**: each `TickRunner.Run` allocates its own `Map` so the cache is per-call by isolation; within a single `Map` the only `FindPath` callers (live `GameScreen`, headless `TickRunner`) are single-threaded, and `SetTile` is only invoked during generation before any pathfinding fires. PathFinder reuse never sees a stale graph.
- **`Tarpg.Tests/Sim/TickRunnerParallelismTests.cs`** — tightened concurrent invocation count (8 → 4 workers), trimmed `[Theory]` cases (3 → 2), and added a `MaxTicks = 3500` cap on every `Run` call. Same thread-safety claim, ~10× faster test runtime — enough for `dotnet test` to finish quickly even under the parallelism contention xUnit adds on top.

### tarpg-sim parallel sweeps
- **`Tarpg.Sim/Program.cs`** — sweep loop now runs each `(floor, seed)` work item via `Parallel.ForEach` with `MaxDegreeOfParallelism = ProcessorCount` by default (overridable via `-p` / `--parallel`, also exposed in `simi`'s prompts). On a typical 8-core machine this cuts a 250-run sweep wall-time roughly 6–8×; on a single-core box (or `--parallel 1`) behavior is identical to the old serial loop.
- **Per-floor banner now fires when that floor's last run lands**, not in floor order — implemented via a `FloorAggregator` per floor with a "did this Add complete the floor?" return value, so the live console output still gets a summary line per depth even though depths interleave. Final aggregate table is sorted by floor before printing, and CSV rows are sorted by `(floor, seed)` so output is byte-stable regardless of worker count or finish order.
- **Why it's safe**: `TickRunner.Run` allocates everything per call (Map, Player, enemies, controllers, Random) so there's no shared mutable state across runs. The only shared bits are `ContentInitializer.Initialize()` (already lock-guarded + idempotent; explicitly called once before parallelism kicks in) and the read-only `Registries` dictionaries (concurrent reads of `Dictionary` are safe as long as no writer is racing).
- **Why not just bump the tick rate instead** (in case someone forgets why this was the safer call): bigger `TickDeltaSec` makes AI react less often per sim-second (a 60Hz kiter is a different opponent than a 15Hz kiter), changes spatial points where sticky-target hysteresis re-evaluates, and risks tunneling the per-axis wall-slide collision when fast movers step >1 tile per tick. Parallelization keeps the sim bit-identical to live play, which is the property that lets us tune off the data.
- **Tests** — `Sim/TickRunnerParallelismTests.cs` is the load-bearing claim: same `(floor, seed)` config produces a bit-identical `SimResult` across 8 concurrent invocations, and a mix of distinct configs run in parallel matches their serial-baseline results pair-for-pair. If a future change introduces shared state into the run path, this test goes red before any sweep results get silently corrupted.

### Greedy pilot sticky targeting + stuck detector + timeout diagnostics
- **Bug found**: first full baseline sweep (`sim --floors 1-10 --seeds 25`) had 8–12% timeouts at F3+. Diagnostic on a stuck run (seed=1009 F3) showed the player walking normally for 30 sim-seconds then freezing at (90, 14) for the remaining 270 sim-seconds.
- **Root cause**: greedy pilot picked nearest enemy by Euclidean distance every tick. With two enemies at near-equal distance from the player (e.g., a pup south at (70, 51) and a wolf east at (129, 30) from (90, 14)), tiny player jitter swapped "nearest" each frame → `RetargetTo` rebuilt conflicting paths (one south, one north) → `step.Y` flipped sign each frame → net player motion zero. Player pinned forever in their current tile.
- **`Sim/GreedySimPilot.cs`** — sticky-target hysteresis: once a target is set, only switch if a candidate is at least 25% closer (in dist²) than the current target. Prevents the alternating-frame target swap that caused the path oscillation.
- **`Sim/GreedySimPilot.cs`** — stuck detector: if the player's tile hasn't changed for 60 ticks (1 sim-second), drop the combat target and route to the threshold instead. Safety net in case some other pathological case ever pins the pilot — the run ends cleanly as Cleared (if it can reach threshold) or as a "real" Timeout meaning "actually unreachable" rather than "pilot got confused."
- **`Sim/TickRunner.cs`** — timeout diagnostics: emit player position trace (1 sample per sim-second) plus a 21×21 ASCII map view around the stuck position. Costs nothing on successful runs; on timeouts prints exactly the data needed to spot a similar root cause.
- **`Movement/MovementController.cs`** — defensive: if A*'s returned path includes the start tile, drop it. `Map.FindPath` already strips it, but a future caller routed directly through RogueSharp wouldn't be safe without this guard.
- **Verified**: re-running seeds 1009 + 1010 across F1-F10 (the worst offenders in the baseline) gives 0 timeouts. Reaver clears F1-F8 100% on those seeds, dies on F9-F10 about half the time. F3 seed=1009 clears in 91.5s with 17 kills (was: 300s timeout with 6 kills).
- **Tests** — `Sim/GreedySimPilotTests.cs` covers nearest-target setting, no-enemies-routes-to-threshold, stuck-bails-to-threshold, stuck-then-moves-resumes-engagement, near-equal-distance-no-thrash, and meaningfully-closer-overrides-sticky. `Movement/MovementControllerTests.cs` adds an every-frame-RetargetTo-with-blocked-LOS regression that pins the corridor-traversal path.
...
src/Tarpg/
  ...
  Sim/
    ...
    GreedySimPilot.cs                 nearest-enemy melee pressure-test pilot (Reaver-flavored)
    KitingSimPilot.cs                 kite-band ranged pilot (Hunter-flavored)
