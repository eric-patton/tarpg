...
> **Updated**: 2026-04-30 (Hunter Focus regen + KitingSimPilot commit-fight)
...
- **Seed 1023 generates an unclearable BSP layout at F6+** — both `Reaver/greedy` and `Hunter/kiting` time out on this seed at floors 6-10 with ~7-8 of 18 enemies killed. Both pilots reach a section of the map and can't path to the remaining enemies. Likely a corridor-thinness or BSP-disconnection edge case that the pilots happen to dead-end in (different geometry from 1018 F2's wall-LOS pin — that one's fixed). Worth a closer look if it shows up on more seeds with deeper sweeps; for now it's a 1-in-25 outlier and a known data point in the baseline.
